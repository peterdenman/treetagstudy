...
def find_deepest_weakness(start_tag):
    """
    Recursively dives down the tree to find the absolute worst 'leaf' node.
    Logic:
    1. Look at all children of start_tag.
    2. Find the worst one.
    3. If that worst child has children of its own, repeat step 1 for that child.
    4. Stop when we hit a tag with no children (or no reviewed children).
    """
    current_tag = start_tag
    current_ret = 100.0
    
    # Safety limit to prevent infinite loops in malformed tag trees
    for _ in range(10): 
        ...
        for child in children:
            # We filter for >3 reviews so we don't flag a new video you just started as "Worst"
        ...
        if found_data and worst_child:
            # We found a path down!
            current_tag = worst_child
            current_ret = worst_score
        else:
            # No children had enough data to judge, or no children existed
            break
            
    # If we didn't move from the start (e.g. root has no reviewed children), return None
...
class ResultsTreeDialog(QDialog):
    def __init__(self, root_tag, parent=None):
        ...
        # --- PRE-CALCULATE HEADER DATA ---
        p, t, r = get_retention_data(root_tag)
        
        # NEW LOGIC: Recursive Search for Deepest Weakness
        ...
        # Right: Recursive Recommendation
        if weakest_tag:
            # Parse names for pretty display
            # e.g. #Bootcamp::Endocrine::Thyroid -> "Thyroid"
            # e.g. parent "Endocrine"
            ...
            # Show "in [Parent Category]" if available
            if parent_context:
